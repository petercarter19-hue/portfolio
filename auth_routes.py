...
def owner_workspace():
    ...
    # Cache-Control is not set here: app.py's existing sitewide after_request
    # hook already marks every text/html response no-cache, must-revalidate
    # (the same behavior owner_workspace.html already gets), and this
    # package must not edit app.py. The private, no-store contract applies
    # to the separate GET /api/v1/owner/home JSON endpoint in owner_routes.py.
    return render_template(
        "owner_home.html",
