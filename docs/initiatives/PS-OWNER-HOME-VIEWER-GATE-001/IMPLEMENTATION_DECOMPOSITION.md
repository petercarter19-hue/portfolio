...
**Outcome:** Versioned owner-only Home endpoint and read-model contract behind a default-off flag. No `/app` or visual change.

**Potential reservations:**

- `app.py` - add only the default-off `PEERSLATE_OWNER_HOME_ENABLED` config.
- `owner_routes.py` - add only the flag-gated JSON endpoint; preserve Capture/Moment/Settings routes.
...
- `SQL FIles/Migrations/proposed/PS-HOME-001_owner_home_reads.sql` - proposed new migration name; confirm availability.
- `SQL FIles/Migrations/proposed/PS-HOME-001_owner_home_reads_rollback.sql` - exact rollback.
- `SQL FIles/Verification/PS-HOME-001_owner_isolation_verify.sql` - structural/behavioral/two-owner verification.
...
**Read-only dependencies:** `auth_routes.py`, `identity.py`, Capture/Moment/Placement services and migrations, `owner_workspace.html`, `owner-app.css`, governing docs.
...
- `auth_routes.py` - minimal flag-on template selection/bootstrap integration, sequential after backend merge.
- `templates/base.html` - only the manager-approved standalone-shell conditional from U1/U3.
- `templates/owner_workspace.html` - read-only exact flag-off fallback through founding-alpha stabilization.
...
**Intersections requiring coordination:** `tests/test_owner_home.py` overlaps Package 1; therefore no concurrent branches. `base.html` is limited to the accepted server-owned standalone-shell conditional. Do not edit `owner_routes.py`, `style.css`, `owner-app.css`, `mobile-nav.js`, or global theme/navigation behavior.
