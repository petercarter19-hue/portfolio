...
Instruction update, later on 2026-07-19: this branch synchronized through current `origin/main` at `5cc5b69346ee354bcc36248f7ee5724ce13c9d08` (Voice governance closeout PR 81). The application behavior remains the released Voice visual-parity commit `864a79d1bc1fc61e62f2d2a544dd54a01ebdcb82`; Azure pipeline 113 / `20260719.21` passed Build and Deploy for that exact application source. The released Voice **Coming later** capability-preview pattern is now the authoritative implementation precedent for future Home/viewer design; this planning update does not claim a new signed-in production validation.
...
| Voice Capture | Owner-only upload/transcription/review/explicit private save; private audio proxy; retry/delete; accepted disabled **Coming later** capability previews for future audiences/destinations/attachments/AI | PR 80 is merged to current main and pipeline 113 succeeded for the exact SHA; earlier signed-in lifecycle validation remains recorded | This planning update did not repeat signed-in production visual validation | Home/viewer design should reuse the truth pattern, not Voice code or data: real silhouette, disabled control, visible **Coming later**, no fabricated result |
