...
| M5 | Decorative 3D props (paper stack, magnifier, stone) in the rail margins | Not implemented as literal renders. Rails carry content cards only; any ambient illustration is a later ChatGPT-lane asset decision, not an implementation invention |
...
| M9 | No mobile/tablet frames in the locked set | §12 reflow rules are the documented adaptation; Pete accepts them on the real build at visual review, or routes a mobile set back to the ChatGPT lane |
| M10 | Dashed connector lines between concern highlight and correction card (image 02) and between selected statement row and the review rail (image 03) | Implement as adjacency + shared accent + programmatic association, not a literal drawn connector; must survive reflow |
