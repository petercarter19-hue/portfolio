...
- Azure PR: `<FILL>` squash-merged at `<FILL>`
- Pipeline: `<FILL>` (Build + Deploy + smoke)
...
- Independent Opus review: Conditional with 2 Blockers, 3 Majors, 6 Minors, 15 test gaps — all corrected; focused recheck: Conditional; both required truth fixes applied exactly as specified with pinning tests at 4e0e17a; optional SQL-side filtering deferred by design.
...
- Production migration: `<FILL: apply output + verify>`; rollback file present and rehearsed.
...
- Merged: `<FILL>`; deployed by pipeline `<FILL>`; migration applied to production `<FILL>`.
- **Enablement:** `PEERSLATE_WORKSHOP_ENABLED=true` set on App Service `peerslate-pete` at `<FILL>` per Pete's explicit decision (friends review immediately). Live behavior verified `<FILL>`.
