...
class VoiceInfrastructureContractTests(unittest.TestCase):
    ...
    def test_container_payload_uses_cross_platform_json_file_and_cleans_it_up(self):
        self.assertIn("[System.IO.Path]::GetTempFileName()", self.script)
        self.assertIn('"--body", "@$ContainerPayloadPath"', self.script)
        self.assertIn(
            "Remove-Item -LiteralPath $ContainerPayloadPath",
            self.script,
        )
        self.assertNotIn(
    ...
    def test_role_queries_avoid_shell_sensitive_length_expression(self):
        self.assertNotIn('"length(@)"', self.script)
        self.assertIn('"--query", "[].id"', self.script)
        self.assertIn("Get-AzureRoleAssignmentCount", self.script)
