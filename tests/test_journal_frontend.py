...
# Reuses the exact same PEERSLATE_CHROME-aware Mac/Windows resolver as the
# three capture harnesses, so this test's Chrome requirement is truly
# identical rather than a second copy that could quietly drift.
...
class JournalEvidenceCaptureContractTests(unittest.TestCase):
    def test_capture_states_name_the_visible_proof_not_only_the_fixture(self):
        expected = {
            "composer-type-desktop-light-1440.png": "composer-type",
            "composer-speak-mobile-dark-390.png": "composer-speak",
            "composer-mobile-save-failure-390.png": "composer-save-failure",
            "composer-mobile-voice-failure-dark-320.png": "composer-voice-permission-failure",
            "timeline-mobile-dark-390-entries-b.png": "timeline-entries-b",
            "timeline-desktop-200pct-zoom-light-1440.png": "timeline-200-percent-css-viewport-reflow",
            "timeline-desktop-forced-colors-1440.png": "timeline-forced-colors",
            "manage-mobile-light-390-footer.png": "manage-footer",
            "detail-mobile-dark-390-lower.png": "detail-lower",
        }
        for filename, state in expected.items():
            with self.subTest(filename=filename):
                self.assertEqual(evidence_state_for_capture(filename, "timeline"), state)

    def test_200_percent_capture_uses_css_viewport_reflow_not_body_zoom(self):
        source = (
            Path(__file__).resolve().parent.parent
            / "scripts"
            / "capture_ps_journal_j1_evidence.py"
        ).read_text(encoding="utf-8")
        zoom_call = re.search(
            r'capture\(\s*"timeline-desktop-200pct-zoom-light-1440\.png",(?P<body>.*?)\n\s*\)',
            source,
            re.DOTALL,
        )
        self.assertIsNotNone(zoom_call)
        self.assertIn("width=720", zoom_call.group("body"))
        self.assertIn("height=520", zoom_call.group("body"))
        self.assertIn("reflow_scale_percent=200", zoom_call.group("body"))
...
class JournalBrowserBehaviorTests(unittest.TestCase):
    ...
    def _assert_load_more_failure_recovers(self, page, expected_announcement):
        page.goto(f"{self.base_url}/app/journal", wait_until="networkidle")
        load_more = page.locator("#journal-load-more")
        original_markup = load_more.inner_html()

        with page.expect_request(lambda request: "cursor=" in request.url):
            load_more.click()
        page.wait_for_function(
            "expected => document.getElementById('journal-live-region').textContent.includes(expected)",
            arg=expected_announcement,
        )
        self.assertFalse(load_more.is_disabled())
        self.assertEqual(load_more.inner_html(), original_markup)
        self.assertEqual(load_more.locator("span").count(), 2)
        self.assertEqual(load_more.locator("[aria-hidden='true']").inner_text(), "⌄")

        # The restored control must actually retry, not merely look enabled.
        with page.expect_request(lambda request: "cursor=" in request.url):
            load_more.click()
        page.wait_for_function("!document.getElementById('journal-load-more').disabled")
        self.assertEqual(load_more.inner_html(), original_markup)

    def test_load_more_503_restores_accessible_markup_and_allows_retry(self):
        page = self._page()
        request_count = []
        ...
        page.route("**/api/journal/moments**", api)
        self._assert_load_more_failure_recovers(
            page,
            "Could not load more moments. Try again.",
        )
        self.assertEqual(len(request_count), 2)

    def test_load_more_rejected_network_restores_markup_and_allows_retry(self):
        page = self._page()
        request_count = []
        ...
        self._assert_load_more_failure_recovers(
            page,
            "Could not load more moments. Check your connection and try again.",
        )
        self.assertEqual(len(request_count), 2)

    def test_200_percent_css_viewport_equivalent_reflows_without_clipping(self):
        page = self._page(width=720, height=520)
        page.goto(f"{self.base_url}/app/journal", wait_until="networkidle")
        page.locator("#journal-open-composer").focus()
            ...
            """() => {
                ...
                const boxes = selectors.map((selector) => {
                    ...
                    return {
                        ...
                        clipped: rect.left < 0 || rect.right > innerWidth,
                        rect: {left: rect.left, right: rect.right, top: rect.top, bottom: rect.bottom},
                    };
                });
                const [headline, action] = boxes;
                const overlaps = !(
                    headline.rect.right <= action.rect.left || action.rect.right <= headline.rect.left ||
                    headline.rect.bottom <= action.rect.top || action.rect.bottom <= headline.rect.top
                );
                return {
                    innerWidth,
        ...
        self.assertEqual(geometry["innerWidth"], 720)
        self.assertEqual(geometry["clientWidth"], 720)
        self.assertLessEqual(geometry["scrollWidth"], geometry["clientWidth"])
        self.assertFalse(geometry["overlaps"])
        self.assertEqual(geometry["focused"], "journal-open-composer")
        for box in geometry["boxes"]:
            with self.subTest(selector=box["selector"]):
